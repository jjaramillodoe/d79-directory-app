...
const Step11MilitaryRecruitmentOptOut = ({ stepData, updateStepData, isActive, currentStep }) => {
  ...
  const renderQuestion = (question) => {
    ...
    if (question.type === 'checkbox') {
      return (
          <div className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg border-2 border-gray-200 hover:border-yellow-400 transition-colors duration-200">
          <input
            type="checkbox"
            id={question.id}
            checked={value}
            onChange={(e) => handleInputChange(question.id, e.target.checked)}
            className="mt-1 w-5 h-5 text-yellow-600 bg-white border-gray-300 rounded-md focus:ring-yellow-500 focus:ring-2 cursor-pointer"
          />
    ...
    if (question.type === 'text') {
      return (
        <input
          ...
          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-200 hover:border-gray-400"
        />
      );
    }
    ...
    return (
      <div>
        ...
        <textarea
          ...
          placeholder={question.placeholder}
          rows={5}
          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-200 resize-none hover:border-gray-400"
        />
      </div>
    );
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-6 mb-6">
          ...
          <div className="flex-1">
            <h2 className="text-3xl font-bold text-white mb-2">
              Section 10: Military Recruitment Opt-Out Notification
            </h2>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 mt-6">
          <div className="text-white space-y-3">
            <p className="leading-relaxed">
              Schools must inform parents/guardians of their right to opt-out of military recruitment and ensure proper documentation of all opt-out requests in accordance with federal and state requirements.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-300 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-lg font-bold text-blue-800">Parental Rights</span>
          </div>
          <p className="text-blue-800 leading-relaxed">
            This step establishes protocols for informing parents about military recruitment opt-out rights and managing opt-out requests properly.
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 border-2 border-yellow-400 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
          ...
          <p className="text-yellow-800 leading-relaxed">
            Ensure compliance with federal requirements and proper documentation of all military recruitment opt-out notifications.
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-6 shadow-lg">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-xl font-bold text-white">Protecting Parental Rights</span>
        </div>
        <p className="text-blue-50 leading-relaxed">
          Military recruitment opt-out notifications protect parental rights and ensure compliance with federal requirements. Ensure all procedures are clearly documented and families are properly informed of their rights.
        </p>
      </div>
    </div>
  );

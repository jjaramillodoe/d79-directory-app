...
Set `GEO_RESTRICT=deny` only after Vercel Firewall **log** rules show no legitimate New York schools matching as out-of-state. Missing geo headers (localhost) are allowed. See [Edge proxy and security](/architecture/security).

...
{/* Screenshot needed: Year setup — Super Admin Year setup (dashboard ?view=bulk-create) with rollover controls, archive toggle, deadlines, and pinned question-bank version. */}
...
1. **One school:** Forms (`/admin/submissions`) → **Make live** on that row. The year stays archived; only that plan is editable (`allowEditsWhenArchived`).

...
| `GET` `POST` | `/api/admin/forms/populate-bank` | Super Admin. Seed a plan from a published question bank. `POST { formId, apply: true, fillAnswers?, force?, label?, version? }`. `fillAnswers: true` writes sample responses for every question. |

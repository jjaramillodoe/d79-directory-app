...
| `rl:export-pdf:{userId}` | 10 | 60s | Yes |
| `rl:export-docx:{userId}` | 10 | 60s | Yes |
| `rl:export-district:{userId}` | 5 | 60s | Yes |
| `rl:clienterr:{ip}` | 10 POSTs | 300s | Yes (`src/proxy.js`, unauthenticated) |
...
<Callout type="note">
  Presence (`/editors`, `/locks`) uses the same `canViewForm` helper as the form GET. Same-school levels 2–4 should see 200. Level 1 still needs assignment or share. See [Troubleshooting](/guides/troubleshooting).
</Callout>

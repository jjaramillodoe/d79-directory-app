...
Public HTML (`/`, `/about`, `/login`) **is** in the matcher so a New York geo deny can cover them, but those paths skip the session redirect (otherwise `/login` would loop). `GET /api/public/*` returns immediately without a session.

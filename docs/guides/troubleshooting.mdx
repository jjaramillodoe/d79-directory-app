...
<AccordionGroup>
  ...
  <Accordion title="403 This application is only available from New York State">
    `GEO_RESTRICT=deny` is on, or a WAF deny rule is live. Allowed geo is US + NY state, not city = New York. Local hosts without Vercel geo headers are allowed. Leave `GEO_RESTRICT` unset until Firewall logs look clean.
  </Accordion>
